...
// @param x,y: start coordinate to draw ascii
// @param color: draw ascii in rgb888 color
...
// @param hz: Chinese char code in GB2312
...
void draw_a_Chinese_ft(void) {

	unsigned long hz_uni = (unsigned long)random_Chinese_unicode();
	unsigned int char_color = random_rgb888();
	unsigned int char_x = random() % info.xres;
	unsigned int char_y = random() % info.yres;

	printf("HZ_UNI: 0x%x, start coordinates: (%d, %d), rgb: 0x%x\n", hz_uni, char_x, char_y, char_color);

	/* 加载 glyph 图像到 slot（会清除之前的 glyph） */
	if (FT_Load_Char(face, hz_uni, FT_LOAD_RENDER)) {
		printf("Failed to load character 0x%x!\n", hz_uni);
	} else {
		if (face->glyph == NULL) {
			printf("face->glyph is NULL after loading\n");
		} else if (face->glyph->bitmap.buffer == NULL) {
			printf("Glyph bitmap is NULL after loading\n");
		} else {
			printf("Glyph bitmap is successfully loaded\n");
	
			// 接下来可以安全地访问 slot->bitmap
			// 例如：unsigned char* buffer = face->glyph->bitmap.buffer;
		}
	}

	slot = face->glyph;	// 原来缺少这行，&slot->bitmap 就会报 segment fault，因为 slot 是空的;

	lcd_put_Chinese_ft(&slot->bitmap, char_x, char_y, hz_uni, char_color);
}
...
int main(int argc, char** argv) {
	...
	munmap(hzk_base, statbuf.st_size);
	close(hzk_fd);
