 ...
 *		-------------------------------------------------------------------------------------------------
 *		6. led_drv5_dtbs_pinctrl_seperate: 分离 led_hrtimer 模块和 led_operations 操作 led 的模块;
 *			6.1 led_operations:
 *					static struct gpio_desc* led_gpio;
 *					...
 ...
 * 					};
 * 				+ struct gpio_desc* 可以被封装进 led_operations; 
 * 				+ 但使用起来是 p_led_opr->func(); 似乎所有的操作，都是对具体的一个 led_gpio 引脚做操作;
 * 			6.2 led_hrtimer:
 * 					struct led_hrtimer {
 ...
 * 			2.2 cp zlj_led_drv.ko 和 led_test 到ubuntu和开发板共享目录下;
 * 			2.3 echo "7 4 1 7" > /proc/sys/kernel/printk，这样能查看我们加的printk;
 * 			2.4 插入模块：
 * 				insmod zlj_led_drv.ko
