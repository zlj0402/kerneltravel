/**
 * @brief: key button, request_irq with driver -- irq content in threaded_irq; -- porting on 2/7/2025
 *
 * @chapter: Notion -- 
 ...
 *
 *			只有上面这三步骤，就结束了很简单...	
 *		9. do something in work_queue(中断下半部)(工作队列，与用户进程共同竞争 CPU 资源，能休眠) --- 第五篇 - 第二十章 - 07_工作队列
 ...
 *			-----------------------------------------------------------------------------
 *		10. 中断服务程序的注册，变为 request_threaded_irq，伴随的中断下半部，会以线程化的方式进行处理;
 *			+ platform.probe 当中对 gpio_keys 进行初始化之后，进行的中断服务程序注册的方式，由 request_irq 变为 request_threaded_irq;
 *				for (i = 0; i < count; i++) {
 *					gpio = of_get_gpio_flags(node, i, &flags);
 *					irq = gpio_to_irq(gpio);
 *					...
 *					err = request_threaded_irq(irq, gpio_key_irq_zlj, key_threaded_func, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "zlj_gpio_keys", &gpio_keys[i]);
 *					tasklet_init(&gpio_keys[i].key_tasklet, key_tasklet_func, (unsigned long)&gpio_keys[i]);
 *					INIT_WORK(&gpio_keys[i].key_work, key_work_func);
 *				}
 *				其他方式的 中断下半段（软件）还是同时存在的，他们当中都只做简单的输出，不做缓冲池的读写;
 *			+ 中断上半段（硬件）部分，原来的中断服务函数，返回状态，要从原来的 IRQ_HANDLED 改为 IRQ_WAKE_THREAD;
 *				static irqreturn_t gpio_key_irq_zlj(int irq, void* dev_id) {
 *					struct gpio_key* gkey = (struct gpio_key*) dev_id;
 *					//printk("%s key %d val %d irq happened\n", __FUNCTION__, irq, gpio_get_value(gkey->gpio));
 *					tasklet_schedule(&gkey->key_tasklet);
 ...
 *				}
 *			+ 给 request_threaded_irq 提供的软件中断下半部的函数 thread_fn;
 *				static irqreturn_t key_threaded_func(int irq, void *dev_id)
 *				{
 *					struct gpio_key* gkey = (struct gpio_key*) dev_id;
 *					int val;
 *					val = gpiod_get_value(gkey->gpiod);
 *					printk("%s: the process is %s %d\n", __FUNCTION__, current->comm, current->pid);
 *					printk("%s gpio %d val %d\n", __FUNCTION__, gkey->gpio, val);
 *					return IRQ_HANDLED;
 *				}
 *							
 * @steps:
 ...
 * 		4. 按下开发板上的 key1, key2
 * 		5. 执行测试程序，按下按键;
 * 			即使没有按下按键，也会有对应的中断下半部的线程;
 * 			我是先按了其中一个，ps 了一下进程，发现有两个 irq/<irq_num><thread->comm> 的进程;
 * 			又去按了另一个按键，发现是对应的线程，所以得出结论，即使没有按下按键，也会有对应的中断下半部线程;
 *			output:
 *				[   75.258070] key_tasklet_func gpio 110 val 1
 *				[   75.262490] key_threaded_func: the process is irq/189-zlj_gpi 309
 *				[   75.268762] key_threaded_func gpio 110 val 1
 *				[   75.275311] key_work_func: the process is kworker/0:1 20
 *				[   75.280711] key_work_func gpio 110 val 1
 *				[   75.284879] key_timer_expire gpio 110 val 1
 *				[   75.435900] key_tasklet_func gpio 110 val 0
 *				[   75.440298] key_threaded_func: the process is irq/189-zlj_gpi 309
 *				[   75.446583] key_threaded_func gpio 110 val 0
 *				[   75.452237] key_work_func: the process is kworker/0:1 20
 *				[   75.457817] key_work_func gpio 110 val 0
 *				[   75.464594] key_timer_expire gpio 110 val 0
 *
 *				ps 不分输出:
 *				308 root     [irq/208-zlj_gpi]
 *				309 root     [irq/189-zlj_gpi]
 *				发现还有个跟 threaded_irq 相关的进程，于是按了开发板上两个按键中的另外一个;
 *
 *				[  152.277793] key_tasklet_func gpio 129 val 1
 *				[  152.282268] key_threaded_func: the process is irq/208-zlj_gpi 308
 *				[  152.288542] key_threaded_func gpio 129 val 1
 *				[  152.295181] key_work_func: the process is kworker/0:1 20
 *				[  152.300579] key_work_func gpio 129 val 1
 *				[  152.304736] key_timer_expire gpio 129 val 1
 *				[  152.451332] key_tasklet_func gpio 129 val 0
 *				[  152.455827] key_threaded_func: the process is irq/208-zlj_gpi 308
 *				[  152.461981] key_threaded_func gpio 129 val 0
 *				[  152.467828] key_work_func: the process is kworker/0:1 20
 *				[  152.473220] key_work_func gpio 129 val 0
 *				[  152.477408] key_timer_expire gpio 129 val 0
