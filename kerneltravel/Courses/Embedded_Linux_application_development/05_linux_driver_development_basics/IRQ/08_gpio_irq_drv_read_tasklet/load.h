/**
 * @brief: key button, request_irq with driver -- irq content in tasklet; -- porting on 1/7/2025
 *
 * @chapter: Notion -- 
 ...
 * 		5. 执行测试程序，按下按键;
 *			output:
 *			[root@100ask:/mnt/05_linux_drivers_development_basics/IRQ/08_gpio_irq_drv_read_tasklet]# ./button_irq_test /dev/zlj_gpio_key
 *			for-loop get button : gpio 110 val 0
 *			for-loop get button : gpio 129 val 0
 ...
 *			get button -1
 *			get button -1
 *			[  622.992503] key_tasklet_func gpio 110 val 1
 *			[  623.014633] key_timer_expire gpio 110 val 1
 *			get button : gpio 110 val 1
 *			[  623.194147] key_tasklet_func gpio 110 val 0
 *			[  623.214634] key_timer_expire gpio 110 val 0
 *			get button : gpio 110 val 0
 *			[  625.573725] key_tasklet_func gpio 110 val 1
 *			[  625.594642] key_timer_expire gpio 110 val 1
 *			get button : gpio 110 val 1
 *			[  625.784252] key_tasklet_func gpio 110 val 0
 *			[  625.804670] key_timer_expire gpio 110 val 0
 *			get button : gpio 110 val 0
 *
 *			首先解释：
 *			测试程序，会首先从缓冲池内取10次数据，后面才是 block 的方式，在 while 循环中读取;
 *			以后真留意到这个 output 会有疑问，为什么会立即有数据，
 *			因为在执行测试程序之前，自己按了 3 次按键，
 *			但还是会有疑问，为什么却有 8 条 printk，不应该只有 6 条吗？
 *			因为刚 insmod 模块时，间隔了一段时间才进行按下按键的操作，为了避免 insmod 时就立即触发一次 timer，设置了:
 *				gpio_keys[i].key_timer.expires = ~0;	// 为了避免一设置定时器，就立即触发一次 timer handler 的调用，所以设置了一个长一点的时间;
 *			有两个注册的按键，所以有两个 0 因为 timer 到时被输出;
 *
 *			tasklet 当中只做了 printk 输出，没有做将数据放入缓冲池中，所以每次按下或松开，测试只输出一次按键引脚的状态;
