...
#define MAX_GLYPHS 32	//	max glyphs per line
...
static int line_box_ymin = 10000;
static int line_box_ymax = 0;

static int line_box_width;
static int line_box_height;
...
void print_glyph_metrics(FT_GlyphSlot* slot) {

	FT_Glyph_Metrics gm = (*slot)->metrics;
	printf("\nface->glyph->metrics (FT_Glyph_Metrics):\n");

	printf("glyph width: %d == %d pixels, height: %d == %d pixels\n", gm.width, gm.width >> 6, gm.height, gm.height >> 6);
	printf("horiBearingX: %d == %d pixels\n", gm.horiBearingX, gm.horiBearingX >> 6);
	printf("horiBearingY: %d == %d pixels\n", gm.horiBearingY, gm.horiBearingY >> 6);
	printf("horiAdvance: %d == %d pixels\n", gm.horiAdvance, gm.horiAdvance >> 6);
}

void print_glyph(void) {
	...
	printf("bbox.xMax: %d, bbox.xMin: %d\n", bbox.xMax, bbox.xMin);
	printf("bbox.yMax: %d, bbox.yMin: %d\n", bbox.yMax, bbox.yMin);
...
int get_glyphs_from_wchars(FT_Face face, wchar_t* str, TGlyph glyphs[]) {

	PGlyph glyph = glyphs;
	FT_Vector pen = {0 * 64, 0 * 64};
	FT_Error error;
	
	for (int i = 0; i < wcslen(str); ++i) {

		unsigned long wchar = (unsigned long)str[i];
		printf("UNI: 0x%x ", wchar);

		glyph->index = FT_Get_Char_Index(face, wchar);	// 获取字形 index

		error = FT_Load_Glyph(face, glyph->index, FT_LOAD_DEFAULT);	// 加载字形对象到插槽: face->glyph
		if (error) {
			
			printf("face->glyph loaded failed.\n");
			continue;
		}
		else {
			printf("face->glyph loaded successfully %d, ", i);
		}

		slot = face->glyph;
	
		error = FT_Get_Glyph(face->glyph, &glyph->image); // 获取字形对象
		if (error) {
			...
			printf("glyph got successfully %d\n", i);
		}

		// -------------------- 对字符 glyph->image 进行偏移 -------------------
		glyph->pos.x = pen.x;
		glyph->pos.y = pen.y;
		printf("            slot->advance.x = %ld\n", slot->advance.x);
		printf("before transform: advance.x = %ld == %ld\n", glyph->image->advance.x, glyph->image->advance.x >> 10);
		FT_Glyph_Transform(glyph->image, NULL, &glyph->pos);
		printf(" after transform: advance.x = %ld\n", glyph->image->advance.x);

		printf("pen.x: %d == %d pixels\n", pen.x, pen.x >> 6);

		// ------------------------ 更改下个字符的偏移值 ------------------------
		// pen.x/y 是 26.6
		// glyph->image->advance.x 是 16.16 单位制
		// slot->advance.x 是 26.6 单位制
		
		// pen.x += glyph->image->advance.x >> 10;
		// 上面等价于
		pen.x += slot->advance.x;	// pen.x/y 是 26.6
...
void compute_string_bbox(TGlyph glyphs[], FT_UInt num_glyphs, FT_BBox  *abbox) {
	...
	for (int i = 0; i < num_glyphs; ++i) {
		...
		printf("%s glyph_bbox.xMin: %d\n", __func__, glyph_bbox.xMin);
...
void lcd_put_Chinese_ft(FT_Bitmap* bitmap, FT_Int x, FT_Int y, unsigned int color) {
	...
	for (int i = 0; i < bitmap->rows; ++i) {
		...
		for (int j = 0; j < bitmap->width; ++j) {
			...
			int cx = x + j;
			if (cx < 0 || cx >= info.xres || cy < 0 || cy >= info.yres)
				continue;
...
void draw_Chinese_line(wchar_t* str) {
	...
	for (int i = 0; i < wcslen(str); ++i) {
		...
		print_glyph_metrics(&slot);
		print_glyph();

		lcd_put_Chinese_ft(&slot->bitmap, char_x, char_y, char_color);
...
void draw_Chinese_line_with_pen(wchar_t* str) {
    ...
    for (size_t i = 0; i < len; ++i) {
        unsigned int wchar = (unsigned int)str[i];
        printf("UNI: 0x%x, pen: (%ld, %ld) == (%ld, %ld), rgb: 0x%x\n", wchar, pen.x, pen.y, pen.x >> 6, pen.y >> 6, char_color);
        ...
        lcd_put_Chinese_ft(&slot->bitmap, draw_x, draw_y, char_color);
...
void draw_glyphs(TGlyph glyphs[], FT_UInt num_glyphs, FT_Vector pen) {
    ...
    unsigned int char_color = random_rgb888();
	printf("%s -- color: 0x%x, num_glyphs: %d\n", __func__, char_color, num_glyphs);

	for (int n = 0; n < num_glyphs; ++n) {

		FT_Glyph_Transform(glyphs[n].image, 0, &pen);
		// convert image to bitmap && destroy glyph copy
		error = FT_Glyph_To_Bitmap(&glyphs[n].image, FT_RENDER_MODE_NORMAL, 0, 1);
		if (!error) {

			FT_BitmapGlyph bit = (FT_BitmapGlyph)glyphs[n].image;
			lcd_put_Chinese_ft(&bit->bitmap, bit->left, info.yres - bit->top, char_color);
			FT_Done_Glyph(glyphs[n].image);
		}
	}
}

void draw_Chinese_line_with_pen_centering(wchar_t* str, int center_id) {
	
	double angle		  = ( 0.0 / 360 ) * 3.14159 * 2;	   /* use 25 degrees	 */

	TGlyph	glyphs[MAX_GLYPHS] = {0};  /* 字形数组 */
	FT_UInt	num_glyphs;
	FT_BBox bbox;

	num_glyphs = get_glyphs_from_wchars(face, str, glyphs);
	printf("get_glyphs_from_wchars: %d\n", num_glyphs);
	
	compute_string_bbox(glyphs, num_glyphs, &bbox);
	// bbox 是 26.6 fixed-point 格式
	line_box_width = bbox.xMax - bbox.xMin;
	line_box_height = bbox.yMax - bbox.yMin;
	printf("line_box_width, line_box_height: (%d - %d, %d - %d) == (%d, %d)\n", 
											bbox.xMax, bbox.xMin, 
											bbox.yMax, bbox.yMin,
											line_box_width, line_box_height);

	pen.x = ((info.xres - line_box_width) / 2)  << 6;
	pen.y -= ((center_id == 1 ? line_box_height / 2 : 0) - font_width * (center_id - 1)) << 6;
...
int init_ft_library(void) {
	...
	// 4. set size
	font_width = 72;
	while (error = FT_Set_Pixel_Sizes(face, font_width, 0)) {
...
int main(int argc, char** argv) {
	...
	wchar_t* chinese_str = L"居中显示fgc";
	wchar_t* sec_line = L"别看，看你自己的";
	...
	// 确定起始坐标
	// lcd_x: 0
	// lcd_y: font_width -> defined in init_ft_library
	// 笛卡尔坐标系
	// x = lcd_x = 0
	// y = info.yres - lcd_y = info.yres - font_width
	...
	pen.y = (info.yres / 2) << 6;
	// 显示第一行 -- 屏幕正中
	draw_Chinese_line_with_pen_centering(chinese_str, 1);
	printf("\n");
	// 显示第二行
	draw_Chinese_line_with_pen_centering(sec_line, 2);
