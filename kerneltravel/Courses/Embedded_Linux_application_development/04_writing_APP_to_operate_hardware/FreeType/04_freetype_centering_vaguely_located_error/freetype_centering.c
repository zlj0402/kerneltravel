...
int get_glyphs_from_wchars(FT_Face face, wchar_t* str, TGlyph glyphs[]) {
	...
	for (int i = 0; i < wcslen(str); ++i) {
		...
		printf("get glyph->index: %lu\n", glyph->index);

		error = FT_Load_Glyph(face, glyph->index, FT_LOAD_DEFAULT);	// 加载字形对象到插槽: face->glyph
		if (error) {
			
			printf("UNI: 0x%x face->glyph loaded failed.\n", wchar);
			continue;
		}
		else {
			printf("UNI: 0x%x face->glyph got %d", wchar, i);
		}
		...
		if (error) {
			...
			printf("UNI: 0x%x get glyph failed.\n", wchar);
			continue;
		}
		else {
			printf("UNI: 0x%x glyph got %d", wchar, i);
		}
		...
		// 按照 glyph->pos 的值进行平移
		// glyph->image 的位置信息，发生改变
		FT_Glyph_Transform(glyph->image, 0, &glyph->pos);

		pen.x += glyph->image->advance.x;	// pen.x/y 是 26.6
...
void draw_Chinese_line_with_pen_centering(wchar_t* str, int center_id) {
	...
	printf("line_box_width: %d, line_box_height: %d\n", line_box_width, line_box_height);

	pen.x = (info.xres - line_box_width / 2) * 64;
	pen.y -= ((center_id == 1 ? line_box_height / 2 : 0) - font_width * (center_id - 1)) * 64;
...
int main(int argc, char** argv) {
	...
	wchar_t* sec_line = L"www.100ask.com";
