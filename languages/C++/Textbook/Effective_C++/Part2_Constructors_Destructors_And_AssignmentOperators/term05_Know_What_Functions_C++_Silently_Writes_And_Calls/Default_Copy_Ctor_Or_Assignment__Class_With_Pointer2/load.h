/**
 * @brief: try to find out if one object(its class with pointer member) which is created by default copy assignment, what does the object's pointer member point to -- 27/3/2025
 * 			ps: 想起来前面，只单纯的证明copy ctor，没证明copy assignment，补一下;
 * 				修改起来，很容易，现象都一模一样;
 *
 * @output:
 ...
 *
 * @summary:
 * 		默认的copy assignment（拷贝赋值），是一字节一字节的复制过来的;包括指针变量存储的地址;
 */
